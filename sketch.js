...
function draw() {
  ...
  const expansion = sin(angle) * maxExpansion;
  const diameter = baseDiameter + expansion;

  const alpha = map(pulseSpeed, 0, 0.25, 0, 220, true);
  const strokeW = map(pulseSpeed, 0, 0.25, 0.5, 6, true);

  if (alpha > 8) {
    noFill();
    stroke(255, alpha); strokeWeight(strokeW);
    ellipse(width / 2, height / 2, diameter, diameter);

    stroke(255, alpha * 0.3); strokeWeight(max(1, strokeW * 0.6));
